-- =====================================================
-- SAFE CONSOLIDATED MIGRATION FOR MYDUB.AI (FIXED)
-- =====================================================
...
-- Indexes for user_profiles (create only if columns exist)
DO $$
BEGIN
    -- Create username index only if column exists
    ...
    -- Create subscription_tier index only if column exists
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_schema = 'public' AND table_name = 'user_profiles' 
               AND column_name = 'subscription_tier') THEN
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'government_services') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tourism_attractions') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tourism_events') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'practical_information') THEN
        DROP TRIGGER IF EXISTS update_practical_information_updated_at ON public.practical_information;
...
DECLARE
    tbl TEXT;
    tables TEXT[] := ARRAY[
        'news_sources', 'news_articles', 'government_departments', 'government_services',
        'tourism_attractions', 'tourism_events', 'user_preferences', 'chat_messages',
        'saved_content', 'search_history', 'page_views', 'user_role_audit',
        'traffic_data', 'weather_data', 'practical_information'
    ];
BEGIN
...
BEGIN
    ...
    -- Government Services
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'government_services') THEN
        DROP POLICY IF EXISTS "gov_services_public_read" ON public.government_services;
    ...
    -- Tourism Attractions
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tourism_attractions') THEN
        DROP POLICY IF EXISTS "tourism_attractions_public_read" ON public.tourism_attractions;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'saved_content') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'traffic_data') THEN
        DROP POLICY IF EXISTS "traffic_data_public_read" ON public.traffic_data;
        CREATE POLICY "traffic_data_public_read" ON public.traffic_data
            FOR SELECT USING (true);
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'weather_data') THEN
        DROP POLICY IF EXISTS "weather_data_public_read" ON public.weather_data;
        CREATE POLICY "weather_data_public_read" ON public.weather_data
            FOR SELECT USING (true);
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'practical_information') THEN
        DROP POLICY IF EXISTS "practical_info_public_read" ON public.practical_information;
...
-- Function to handle new user registration (with column checks)
