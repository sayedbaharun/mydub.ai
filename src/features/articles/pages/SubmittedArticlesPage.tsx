...
export default function SubmittedArticlesPage() {
  const { data, isLoading, error } = useArticles({ status: 'submitted' })
  ...
  if (isLoading) return <p>Loading submitted articles...</p>
  if (error) return <p className="text-red-600">Failed to load submitted articles</p>

  const actions = (a: any) => (
    <div className="flex gap-2 justify-end">
      <Button size="sm" variant="outline" onClick={() => setPreview(toPreviewArticle(a))}>Preview</Button>
      {canTransition(role as UserRole, 'submitted', 'approved') && (
        <Button size="sm" onClick={async () => {
          await transition.mutateAsync({ id: a.id, to: 'approved', currentUserId: user?.id })
          toast.success('Approved')
        }}>Approve</Button>
      )}
      {canTransition(role as UserRole, 'submitted', 'changes_requested') && (
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">Submitted Articles</h1>
      </div>
